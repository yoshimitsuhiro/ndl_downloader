...
; /////////////////////////////////
; /////Step 1: Initiate hotkey/////
; /////////////////////////////////

; Code to use single hotkey to start and stop loop taken from: https://www.autohotkey.com/docs/FAQ.htm#repeat

#MaxThreadsPerHotkey 3  ; allow more than one key press
^F10::  ; Startup hotkey (change this to suit your preferences)

#MaxThreadsPerHotkey 1  ; return to one key press

if StartupHotkeyOn  ; This means an underlying thread is already running the loop below.
{
    StartupHotkeyOn := false  ; Signal that thread's loop is to stop.
    return  ; End this thread so that the one underneath will resume and see the change made by the line above.
}

;Otherwise:
StartupHotkeyOn := true

; ///////////////////////////////////////////////////////////////////////
; /////Step 2: Get total and starting page numbers through input box/////
; ///////////////////////////////////////////////////////////////////////

InputBox, NumOfPages, NDL Screenshoter, Input the total number of pages in the book.
if ErrorLevel
    return
else
;NumOfPages = 1  ; manually set the number of pages you wish to screenshot here

InputBox, PageNum, NDL Screenshoter, Input the page number you wish to start taking screenshots from.
if ErrorLevel
    return
else
;PageNum = 1  ; manually set the number of pages you wish to screenshot here

Sleep, 500  ; necessary to ensure that inputbox is fully closed before capturing screenshot

; ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
; /////Step 3: Calculate the black borders around images using bordercrop.py (python dependencies: opencv, numpy)/////
; ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

FileName = ndl_temp.png  ; keep as png since jpg compression artifacts will affect border detection

CaptureScreen(0, False, FileName, "") ; Capture fullscreen screenshot

RunWait, python.exe bordercrop.py ; set python directory accordingly

Sleep, 500  ; necessary to ensure that command prompt is fully closed before capturing screenshot

FileRead, dimensions, ndl_dimensions.txt

FileDelete, ndl_temp.png
FileDelete, ndl_dimensions.txt

; //////////////////////////////////////////////////
; /////Step 4: Capture screenshot of first page/////
; //////////////////////////////////////////////////

StrPageNum = % SubStr("000" . PageNum, -3)
FileName = %StrPageNum%.jpg

ImgQual = 90  ; set JPEG image quality (for optimal quality use 90, although any value above 70 should be acceptable; qualities above 90 provide no noticable improvement)

CaptureScreen(dimensions, False, FileName, ImgQual) ; Capture calculated coordinates
;CaptureScreen(0, False, FileName, ImgQual) ; Capture fullscreen screenshot (use if for some reason border calculation isn't working)
;CaptureScreen("0, 0, 0, 0", False, FileName, ImgQual) ; Capture specific coordinates (ditto; enter the coordinates you wish to capture)

; /////////////////////////////////////////////////////////////
; /////Step 5: Capture screenshots of each subsequent page/////
; /////////////////////////////////////////////////////////////

Loop, % NumOfPages - PageNum
{
    ; The next group of lines are the action you want to repeat:
	Send, {Right}  ; flip to next page
	
	Sleep, 4000  ; necessary to ensure that page is fully loaded before capturing screenshot (modify as seen fit, but keep above 4000 to avoid capturing the toolbar)
	
	PageNum += 1
	StrPageNum = % SubStr("000" . PageNum, -3)
	FileName = %StrPageNum%.jpg

	CaptureScreen(dimensions, False, FileName, ImgQual) ; Capture specific coordinates
	;CaptureScreen(0, False, FileName, ImgQual) ; Capture fullscreen screenshot (use if for some reason border calculation isn't working)
	;CaptureScreen("0, 0, 0, 0", False, FileName, ImgQual) ; Capture specific coordinates (ditto; enter the coordinates you wish to capture)

    ; Leave the rest below unchanged
    if not StartupHotkeyOn  ; The user signaled the loop to stop by pressing the startup hotkey again
        break  ; Break out of this loop
}
StartupHotkeyOn := false  ; Reset in preparation for the next press of this hotkey.
MsgBox, 0, NDL Screenshoter, Done`!
return

; ///////////////////////////////////////
; /////CaptureScreen by Linear Spoon/////
; ///////////////////////////////////////

; Downloaded from: https://www.autohotkey.com/board/topic/121619-screencaptureahk-broken-capturescreen-function-win-81-x64/
...
;CaptureScreen(aRect = 0, bCursor = False, sFile = "", nQuality = "")  ; default settings
CaptureScreen(aRect, bCursor, sFile, nQuality)
{
	If !aRect
	{
